...
Knight::Knight(const AEVec2 & spawn_location)
	: Characters(S_CreateSquare(KNIGHT_SCALE, ".//Textures/Knight.png"),
	  HEALTH, Col_Comp{ spawn_location.x - KNIGHT_SCALE, spawn_location.y - KNIGHT_SCALE , 
...
void Knight::Attack(Dragon& d, const float dt)
{
	...
	stab.Projectile::Update(STAB_SCALE); // update positon
	
	if (Get_Direction() == RIGHT)
		stab.Transform_.SetRotation(180);    // rotate texture to look right
	else
		stab.Transform_.SetRotation(0);      // rotate texture to look left
