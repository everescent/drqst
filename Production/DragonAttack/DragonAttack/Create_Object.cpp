...
AEGfxVertexList* CreateSquare(float   size, float scaleU, 
                              float scaleV, unsigned int color)
{
  AEGfxMeshStart();
  AEGfxTriAdd(
...
Sprite S_CreateSquare(float   size, const char* texFile, 
                      ...
                      unsigned int                color  )
{
  ...
  if (texFile)
  {
    Sprite tmp_Square( CreateSquare(size, scaleU, scaleV, color), 
                       texFile, size, size                        );
    return tmp_Square;
  }
  else
...
Sprite S_CreateRectangle(float  width, float height, const char* texFile, 
                         float scalex, float scaley, unsigned int  color  )
{
  //Check for texture file
  if (texFile)
  {
    Sprite tmp_Rectangle( CreateRectangle(width, height, scalex, scaley, color), 
...
Sprite CreatePlatform(float  MultW, float  MultH, const char* texFile, 
                      float scalex, float scaley, unsigned int  color  )
{
  //Check for texture file
  if (texFile)
...
Sprite CreateFloor(float  MultW, const char* texFile, 
                   float scalex, float        scaley, 
                   unsigned int color                 )
{
  //Check for texture file
...
Sprite CreateBG(float  MultW, float  MultH, const char* texFile, 
                float scalex, float scaley, unsigned int color   )
{
  //Check for texture file
  if (texFile)
